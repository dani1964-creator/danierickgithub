-- FUNÇÃO RPC CORRIGIDA USANDO AS COLUNAS CORRETAS
    ...
    COALESCE(
      jsonb_agg(
        jsonb_build_object(
          ...
          'slug', p.slug,
          'price', p.price,
          'location', COALESCE(p.address, p.neighborhood || ', ' || p.city), -- USAR address no lugar de location
          'property_type', p.property_type,
          'transaction_type', p.transaction_type,
          'bedrooms', p.bedrooms,
          'bathrooms', p.bathrooms,
          'area', p.area_m2, -- USAR area_m2 no lugar de area
