-- Add unique constraint to website_slug to prevent duplicates
ALTER TABLE public.brokers 
ADD CONSTRAINT unique_website_slug UNIQUE (website_slug);

-- First, let's update the conflicting slugs to make them unique
-- Keep the first one as 'home' and update the second one
