...
import { Building2, Users, Globe, Eye, EyeOff, ExternalLink, RefreshCw, LogOut } from "lucide-react";
...
export default function SuperAdminPage() {
  ...
  // Função SUPER SIMPLES para buscar brokers
  const fetchBrokers = async () => {
    try {
      console.log('🔍 Iniciando fetchBrokers...');
      setLoading(true);
      ...
      console.log(`✅ Brokers encontrados: ${brokersData.length}`);
      brokersData.forEach((b, i) => {
        console.log(`  ${i+1}. ${b.business_name} (${b.email}) - ${b.is_active ? 'Ativa' : 'Inativa'}`);
      });

      // Buscar contagem de propriedades de forma simples
      console.log('🔍 Buscando contagem de propriedades...');
      const brokersWithCounts = [];
      ...
      for (const broker of brokersData) {
        ...
        const brokerWithCount = { ...broker, properties_count: count || 0 };
        brokersWithCounts.push(brokerWithCount);
        console.log(`  ${broker.business_name}: ${count || 0} propriedades`);
      }

      console.log(`✅ Total: ${brokersWithCounts.length} brokers com contagens`);
      setBrokers(brokersWithCounts);
      
    } catch (error) {
      console.error('❌ Erro em fetchBrokers:', error);
      toast({
  ...
  // Função SUPER SIMPLES para toggle status
  const toggleBrokerStatus = async (brokerId: string, currentStatus: boolean) => {
    try {
      console.log(`🔄 Toggle status: ${brokerId} de ${currentStatus} para ${!currentStatus}`);
      ...
      console.error('❌ Erro em toggleBrokerStatus:', error);
      toast({
  ...
  useEffect(() => {
    ...
    if (token === "1") {
      console.log('✅ Token válido, fazendo login...');
      setIsAuthorized(true);
      fetchBrokers();
    } else {
  ...
  console.log(`  Brokers no estado:`, brokers.map(b => `${b.business_name} (${b.is_active ? 'Ativa' : 'Inativa'})`));
      ...
      <div className="container mx-auto p-3 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
          <div className="flex items-center gap-4">
            <h2 className="text-lg sm:text-xl font-semibold">Gerenciar Imobiliárias ({brokers.length})</h2>
          </div>
        ...
        <Card className="mb-4 bg-gray-50">
          <CardContent className="p-4">
            ...
            <p className="text-sm">Brokers carregados: {brokers.length}</p>
            <p className="text-sm">Estado: {JSON.stringify(brokers.map(b => ({ nome: b.business_name, email: b.email, ativa: b.is_active })), null, 2)}</p>
          </CardContent>
        </Card>
